...
slug: multisim-simdf
categories: ["rstats"]
tags: ["r", "simulation", "multisim"]
---

```{r libraries, message=FALSE}
# devtools::install_github("debruine/multisim")
library(multisim)
library(tidyverse)
```

I added a new function to the package [`multisim`](https://github.com/debruine/multisim) to generate a new dataframe from an existing dataframe, simulating all numeric columns from normal distributions with the same mean and SD as the existing data and the same correlation structure as the existing data.
