...
You've probably been directed here because you asked someone about how to test the normality of predictors in an analysis. In this blog post, I'm going to use data simulation to explain why it's only the normality of *residuals* that matters and how you can visualise this with QQ plots.
...
N.B. If you're more used to [simulating data using model parameters](https://debruine.github.io/tutorials/sim-data.html#intercept-model), this way might make more sense to you, but it's often difficult to figure out what the parameters should be if you don't already have pilot data.
...
It's quite hard to judge normality from a density plot, especially when you have small samples, so we can use a QQ plot to visualise how close a distribution is to normal. 
...
So what does our bimodal energy data look like? This sort of QQ plot is less normal (see how the points move away from the line at the ends), but that doesn't matter for your model at all.
...
And you can see that weight is extremely non-normal.
...
ggplot(data, aes(sample = weight)) +
  ...
  labs(x = "Theoretical distribution",
       y = "Sample distribution",
       title = "QQ Plot for Weight")
```

## Statistical Tests

I don't recommend using statistical tests for normality. Essentially, they are underpowered in small samples and overpowered in large samples. [Robert Greener has a good discussion of this.](https://towardsdatascience.com/stop-testing-for-normality-dba96bb73f90)

You can see from the plots above that the energy values are not normally distributed, and this is what the tests for normality would also show.
...
However, the residuals do "pass" the Shapiro-Wilk normality test.
