...
In addition to generating a random intercept for each subject, we will also generate a random slope for any within-subject factors. The only within-subject factor in this design is `stim_version`. The main effect of `stim_version` is set to 50 above, but different subjects will show variation in the size of this effect. We'll set `sub_version_sd` below to the SD of this variation and use this to calculate the random slope (`sub_version_slope`) for each subject.

Also, it's likely that the variation between subjects in the size of the effect of condition is related in some way to between-subject variation in the intercept. So we want the random intercept and slope to be correlated. 

I'm writing a package that has functions to easily create correlated variables for simulations.  The function `rnorm_multi` generates `n` observations of `vars` variables with `mu` means and `sd` standard deviations with `cors` specific correlations. 

You can learn more about this function at my [Simulating Multiple Vectors](rnorm_multi) tutorial. You can use `devtools::install_github("debruine/faux")` to install the package.
...
If you don't want to install the package, you can just include the simplified function in the code chunk below. (The function in the package has more error-checking to prevent you from entering impossible values like correlations > 1)
...
Here, we'll simulate a case where subjects who have slower (larger) reaction times across the board show a smaller effect of condition, so we set `sub_i_version_cor` to a negative number (-0.2).
...
<p class="info">See the [Simulating Multiple Vectors](rnorm_multi) tutorial for more other ways to set the correlations.</p>
...
Here, we're simulating different SDs for different effects, so our plot should reflect this.
...
ranef(mod)$sub_id %>%
  ...
  select(mod_i, sim_i = sub_i, 
         mod_version_slope, sim_version_slope = sub_version_slope) %>%
...
ranef(mod)$stim_id %>%
  ...
  select(mod_i, sim_i = stim_i, 
         mod_version_slope, sim_version_slope = stim_version_slope, 
         mod_cond_slope, sim_cond_slope = stim_cond_slope, 
         mod_cond_version_slope, sim_cond_version_slope = stim_cond_version_slope) %>%
  GGally::ggpairs(lower = list(continuous = "smooth"),
                progress = FALSE)
```
...
See what happens when you increase the SD for the effect of version within subjects.

```{r sim-lmer-changes, eval=FALSE}
sim_lmer(sub_version_sd = 50)
```
