...
You can use the `resid()` function to get the `r glossary("residual", "residual error")` term from your model. This is the difference between the predicted value (based on the weight and species for each subject and the model parameters) and the actual value. Those values should be normally distributed.
