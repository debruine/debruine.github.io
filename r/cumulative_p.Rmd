...
[Download the Rmd notebook for this example](cumulatiove_p.Rmd)

EJ Wagenmakers started an interesting debate last nioght with a [twitter poll on p-values](https://twitter.com/EJWagenmakers/status/889987997046910978). Some responses suggested you can multiply p-values from several tests to create a sort of [cumulative p-value](https://twitter.com/VladMalik/status/890246773515722752) that is the joint p[robability of the null hypothesis.

I also used to believe that you could multiply p-values, but am now a bit embarassed at my misunderstanding, common as it is. The p-value is *not* the probability that the null hypothesius is true, it is the probility of obtaining the current (or more extreme) values under the null hypothesis. This important distinction means you cannot multiply p-values to obtain the probability of several tests.
...
ggsave("cum_p.png", width=6, height=4)
```
